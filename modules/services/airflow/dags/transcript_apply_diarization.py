...
def transcript_apply_diarization():
    ...
    conf_diarization_file = "'artifacts/tracks/{{params.track_id}}/audio/diarization.json'"
    ...
    def download_transcript(
        ...
        return bucket_download_json_data.function(
            f"artifacts/tracks/{track_id}/transcripts/{speaker_info['language']}/raw_response.json"
        )

    @task(
        task_display_name="⬆️ Bucket: Upload Transcript")
    def upload_transcript(
        ...
        bucket_upload_data.function(
            f"artifacts/tracks/{track_id}/transcripts/{speaker_info['language']}/diarized.json",
            transcript)
    ...
    diarization_file = bucket_download_json_data.override(task_display_name="⬇️ Bucket: Download Diarization")(conf_diarization_file)
    speakers         = get_speakers_info(diarization_file)
    transcripts      = download_transcript.partial(track_id=conf_track_id).expand(speaker_info=speakers)
    transcript       = merge.expand(speaker_and_transcript=speakers.zip(transcripts))
    upload_transcript.partial(track_id=conf_track_id).expand(speaker_and_transcript=speakers.zip(transcript))
