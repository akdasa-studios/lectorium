...
from tasks.deepgram import deepgram_transcribe_audio_file
from tasks.bucket import bucket_sign_url, bucket_upload_data
...
def transcript_transcribe_audio():
    ...
    #### Input File:
    - `library/audio/{audio_type}/{track_id}.mp3`: Audio file to process
    - `artifacts/tracks/{track_id}/audio/{audio_type}.mp3`: Isolated speaker audio file

    #### Outputs:
    - `artifacts/tracks/{track_id}/transcripts/{language}/raw_response.json`: Raw response from the API
    ...
    conf_bucket_key_raw_response = "{{ 'artifacts/tracks/' ~ params.track_id ~ '/transcripts/' ~ params.language ~ '/raw_response.json' }}"
    conf_bucket_key_transcript   = "{{ 'artifacts/tracks/' ~ params.track_id ~ '/transcripts/' ~ params.language ~ '/original.json' }}"
    ...
    def get_audio_file_object_key(
        ...
        if audio_type in ["original", "normalized"]:
            return f"library/audio/{audio_type}/{track_id}.mp3"
        elif audio_type.startswith("speaker_"):
            return f"artifacts/tracks/{track_id}/audio/{audio_type}.mp3"
        else:
