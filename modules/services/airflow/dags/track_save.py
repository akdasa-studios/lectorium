...
def track_save():
    ...
    conf_languages = "{{ params.languages }}"
    ...
    @task()
    def get_languages(dag_run: DagRun):
        return dag_run.conf.get("languages", [])

    @task(
        task_display_name="📜 Download Transcript",
        map_index_template="{{ task.op_kwargs['language'] }}")
    def download_transcript(
        track_id: str,
        language: str,
    ):
        transcript_types = ["proofread", "translated", "original"]
        for transcript_type in transcript_types:
            transcript = bucket_download_json_data.function(
                object_key=f"artifacts/tracks/{track_id}/transcripts/{language}/{transcript_type}.json",
