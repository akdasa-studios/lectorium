...
async function getTracks() {
  ...
  for (const track of inboxTracks) {
    const r = await Promise.all((track.references.normalized || []).map(async x => ({
    ...
    result.push({
      ...
      author: {
        value: await normalizeAuthor(track.author.normalized ?? track.author.extracted),
        annotations: await annotateAuthor(track.author.normalized)
      },
      location: {
        value: await normalizeLocation(track.location.normalized ?? track.location.extracted),
