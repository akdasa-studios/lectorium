...
export function useTrackToSearchResultMapper(
  ...
  > {
    ...
    return {
      trackId: track._id,
      title: mapTrackTitle(track.title, options.language),
      author: await mapAuthorFullNameById(track.author, options.language), 
      location: await mapLocationFullNameById(track.location, options.language),
      tags: (track.tags || []).length >= 1
        ? await Promise.all((track.tags || []).map(tag => mapTagFullNameById(tag, options.language)))
        : [],
      references: track.references?.length >= 1 
        ? await Promise.all(track.references.map(ref => mapReference(ref, options. language)))
        : [],
